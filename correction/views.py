...
def create_activity(request, id):
    ...
    activity = Activity.objects.create(name=name,id_class=id_class,description=description, question=question)
    activity.save()
...
def create_answer(request, id):
    ...
    student = get_object_or_404(Student, id=id_student)  # Substitua pela busca do modelo Student
    ...
    answer = Answer.objects.create(id_activity=id_activity,id_student=student,studentAnswer=studentAnswer,score=score,feedback=feedback, extract_value = extract_value)
    answer.save()
    return redirect('select_activity', id=id)
